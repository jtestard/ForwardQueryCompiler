...
public class TestVaryNetworkLatency extends AbstractSetProcessableTestCaseNew
{
    private static final Logger        log                  = Logger.getLogger(TestVaryNetworkLatency.class);
    ...
    private static final List<Integer> TOTAL_LATENCYS       = new ArrayList<Integer>();
    
    static
    {
        TOTAL_LATENCYS.add(0);
        TOTAL_LATENCYS.add(500);
        TOTAL_LATENCYS.add(1000);
        TOTAL_LATENCYS.add(1500);
        TOTAL_LATENCYS.add(2000);
//        TOTAL_LATENCYS.add(10000);
    }
    static
    {
...
//        explain(EXPLAIN_AP_FILE_NAME);
    ...
    private static void run(String query_file_name, boolean rewrite) throws Exception
    {
        ...
        {
            for (int latency : TOTAL_LATENCYS)
            {
                //JdbcExecution.m_sleeptime = latency;
                double time = executeEndToEnd(query_file_name, null, rewrite);
                sum += time;
                results.add(new Object[] { query_file_name, time, latency });
            }
        }
